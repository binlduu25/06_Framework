...
@RestController // 비동기 요청 처리 + controller
@CrossOrigin(origins = "http://localhost:5173") 
// CrossOrigin : 스프링에서 제공하는 어노테이션으로 CORS 설정을 위해 사용됨
// CORS(Cross-Origin Resource Sharing)
// 클라이언트와 서버가 서로 다른 출처(origin)에서 요청을 주고 받을 때 발생하는 보안 정책
// 브라우저에서는 기본적으로 다른 출처(도메인, 프로토콜, 포트 등)의 요청을 차단함
// -> 클라이언트와 서버의 출처가 다를 땐 cors 설정을 적절히 해주어야 정상적인 http 사용 가능
public class MyAppController {
	...
	public String getUserInfo(@RequestBody Map<String, Object> map) {
		// message return 할 것
		// 요청 데이터 중 name값이 홍길동이고 age가 20이면 "홍길동님은 20세입니다"
		// 같지 않다면 "데이터가 없습니다" return
