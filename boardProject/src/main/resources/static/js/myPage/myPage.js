...
if(secession != null) { // form 태그가 화면상에 존재 할 때
...
/*  [input type="file" 사용 시 유의 사항]
  ...
  4. 이 부분은 img 쪽과 file 부분을 구분해서 생각할 필요!
     또한 이 부분 복습 필요
*/
...
imageInput.addEventListener("change", () => {
   ...
   if(file) { // 파일이 선택된 경우
       ...
       if(file.size <= MAX_SIZE) { // 파일 크기가 허용범위 이내인 경우
   ...
   } else { // 파일 선택이 취소된 경우(file 에 들어온 값이 없을 때)
		...
		if(previousFile) {
       		const dataTransfer = new DataTransfer(); // DataTransfer 객체 생성 후 ,
			dataTransfer.items.add(previousFile); // 이전 파일 정보(previousFile)를 저장
