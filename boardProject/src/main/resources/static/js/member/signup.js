...
memberEmail.addEventListener("input", e => {
    ...
    // 5) 유효한 이메일 형식인 경우 중복 검사 수행 (수업 내 진행함)
     // 비동기(ajax)
    ...
    fetch("/member/checkEmail?memberEmail=" + inputEmail)
		...
		.then((count) => {
			...
			if(count == 1){ // 중복 o
				emailMessage.innerText = "이미 사용중인 이메일";
				emailMessage.classList.add("error");
				emailMessage.classList.remove("confirm");
				checkObj.memberEmail = false;
				return;
			}else{ // 중복 x
				emailMessage.innerText = "사용 가능한 이메일"
...
/* 이메일 인증 */
	...
	sendAuthKeyBtn.addEventListener("click", () => {
	    ...
	    // 메일은 비동기로 서버에서 보내면서 화면에서는 타이머 시작하기
        ...
        authKeyMessage.innerText = `${addZero(min)}:${addZero(sec)}`; // 아래 addZero function
...
/*
 ...
 -> 입력된 인증번호와 발급된 인증번호가 같은지 비교(같으면 1, 아니면 0반환)
...
memberNickname.addEventListener("input", (e) => {
    ...
    // 3) 중복 검사 ajax요청 (유효한 경우)
	 // get 방식으로 보내고, queryString 이용해 작성한 닉네임을 가져간다
   fetch("/member/checkNickname?memberNickname=" + inputNickname)
   .then(resp => resp.text())
   .then(count => {
		if(count == 1){ // 중복 o
			nickMessage.innerText = "이미 사용 중인 닉네임";
			nickMessage.classList.add("error");
			nickMessage.classList.remove("confirm");
			checkObj.memberNickname = false;
			return;
